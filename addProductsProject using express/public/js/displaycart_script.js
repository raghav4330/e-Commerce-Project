...
function updateStoredCartProducts(ob,callback)
{
     ...
     request.addEventListener('load', function() 
	  ...
	  request.open('POST', '/updatecartproduct_from_displaycartpage');
      request.setRequestHeader("Content-type", "application/json");
    ...
    for(var j=0;j<cartproducts.length;j++)
        {
            ...
            ordered_data.push({name:products[i].name,cquantity:cartproducts[j].cquantity,price:products[i].price});
